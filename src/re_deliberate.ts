...
import { writeConstraintUpdate } from './plato_bridge';

/**
 * Default delta values for each constraint direction change
 */
const DEFAULT_DELTAS: Record<string, number> = {
 ...
 */
export async function reDeliberate(
  pattern: OverridePattern,
  model: MutableConstraintModel
): Promise<ReDeliberateResult> {
  ...
  await writeConstraintUpdate(pattern, previousValue, newValue, model);
 ...
 */
async function signalCaptainReDeliberation(
  pattern: OverridePattern,
  previousValue: number,
  newValue: number
): Promise<void> {
